...
public interface UserRepository extends JpaRepository<User, UUID> {
     ...
     */
    default Optional<User> findByIdAndIsActiveTrue(UUID id) {
        return findById(id).filter(u -> u.getStatus() == UserStatus.ACTIVE && !u.getDeleted());
    }

    /**
     * Find active user by email
     */
    default Optional<User> findByEmailAndIsActiveTrue(String email) {
        return findByEmail(email).filter(u -> u.getStatus() == UserStatus.ACTIVE && !u.getDeleted());
    }
    ...
    default boolean existsByEmailAndIsActiveTrue(String email) {
        return findByEmailAndIsActiveTrue(email).isPresent();
    }

    /**
     * Check if active user exists by role
     */
    default boolean existsByRoleAndIsActiveTrue(UserRole role) {
        return findByRole(role).stream()
            .anyMatch(u -> u.getStatus() == UserStatus.ACTIVE && !u.getDeleted());
    }
}
